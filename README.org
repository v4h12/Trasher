...
- You can decide whether you want to restore or delete anything with arguments: 'r/d'.
- Either select all or individual files which are numbered within the list.
- Type in which number you'd like to restore/delete and hit enter to let Trasher do its thing!
...
- Updated install script (for argcomplete package and pip)
- Delete from anywhere /using fzf/
- iOS like trash (delete file within /x/ amount of time)
- Modularity
...
#+begin_quote
There is a current refactor in progress in order to provide modularity all round, as well as neatening up code and comments. The overall plan is to make it as simple as possible whilst still allowing you freedom.
#+end_quote
